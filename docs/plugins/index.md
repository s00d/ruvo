...
Enable with Cargo features on `ruvo`. Presets:

```bash
cargo add ruvo --features web   # HTML apps
cargo add ruvo --features api   # JSON APIs
cargo add ruvo --features cors,session,db
```
...
| [`env`](/plugins/env) | Cascade .env loading for Ruvo apps (dotenvy) | `env` |
...
| [`quic`](/plugins/quic) | QUIC datagrams BackgroundService helpers for Ruvo | `quic-udp` |
...
| [`sse`](/plugins/sse) | Server-Sent Events helpers for Ruvo (channels, Last-Event-ID, keep-alive) | `sse-feed` |
...
| [`store`](/plugins/store) | KvStore trait + memory / file / sql / redis backends for Ruvo | `store`, `store-crypto`, `store-file`, `store-redis`, `store-sql` |
...
| [`udp`](/plugins/udp) | UDP BackgroundService helpers for Ruvo | `udp` |
...
| PAT (`rvpat_…`) | `auth_api_tokens` | machine / CI |
...
URL: `DATABASE_URL` or `[db] url`. CLI: `migrate` / `seed` (`cargo ruvo db …`). SQL KV/queue backends reuse the same `DbPool`. See [db](/plugins/db).
