...
| [`auth`](/plugins/auth) | `0.1.4` | Register/login, verify, reset, 2FA, profile, roles | `auth`, `auth-activity`, `auth-mail`, `auth-vld` |
...
| [`csrf`](/plugins/csrf) | `0.1.2` | Session double-submit CSRF (Laravel-style except/XSRF cookie) | `csrf` |
...
| [`notifications`](/plugins/notifications) | `0.1.2` | DB inbox, channels with ACL, optional WS/mail | `notifications`, `notifications-auth`, `notifications-mail`, `notifications-templates`, `notifications-ws` |
...
| [`quic`](/plugins/quic) | `0.1.1` | QUIC datagrams BackgroundService helpers for Sova | `quic-udp` |
| [`rate-limit`](/plugins/rate-limit) | `0.1.1` | Per-key request rate limiting | `rate-limit` |
| [`redis`](/plugins/redis) | `0.1.1` | Shared Redis/Valkey connection for KvStore, tasks, cache, pub/sub, queues | `redis` |
| [`session`](/plugins/session) | `0.1.2` | Cookie sessions backed by a SessionStore | `session`, `session-redis`, `session-sql` |
| [`shield`](/plugins/shield) | `0.1.1` | Baseline security response headers (helmet-style) | `shield` |
| [`sse`](/plugins/sse) | `0.1.1` | Server-Sent Events helpers for Sova (channels, Last-Event-ID, keep-alive) | `sse-feed` |
| [`static`](/plugins/static) | `0.1.1` | Serve files from a directory under a mount path | `static-files` |
| [`storage`](/plugins/storage) | `0.1.1` | Object storage (local / memory / S3 / GCS / Azure) | `storage`, `storage-azure`, `storage-gcs`, `storage-memory`, `storage-s3` |
| [`store`](/plugins/store) | `0.1.1` | KvStore trait + memory / file / sql / redis backends for Sova | `store`, `store-crypto`, `store-file`, `store-redis`, `store-sql` |
| [`tasks`](/plugins/tasks) | `0.1.1` | Job worker, priorities, and optional cron/interval scheduler | `tasks` |
| [`tasks-store`](/plugins/tasks-store) | `0.1.1` | TaskStore trait + memory / file / sql / redis backends | `tasks-file`, `tasks-redis`, `tasks-sql`, `tasks-store` |
| [`templates`](/plugins/templates) | `0.1.1` | MiniJinja HTML templates with optional autoreload | `templates` |
| [`udp`](/plugins/udp) | `0.1.1` | UDP BackgroundService helpers for Sova | `udp` |
| [`vld`](/plugins/vld) | `0.1.3` | Request validation hooks and coverage check | `vld`, `vld-flash`, `vld-flash-templates`, `vld-form`, `vld-i18n`, `vld-openapi` |
| [`ws`](/plugins/ws) | `0.1.1` | WebSocket hub, origin allowlist, max message size | `ws` |
| [`cli`](/plugins/cli) | `0.1.1` | CLI ServerArgs / listen_args for Sova (local dev) | — |
<!-- /generated:plugins-table -->
