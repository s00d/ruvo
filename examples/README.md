...
| | `api_jwt` | `JwtAuth` + PAT `/auth/tokens` + `/api/me` |
| | `api_oauth` | GitHub / Google / optional Apple OAuth + JWT |
| | `crud` | SeaORM Postgres |
| **realtime** | `sse`, `sse_feed`, `ws_chat` | SSE / WebSocket |
| **net** | `udp_echo`, `quic_udp_echo`, `tls_hello` | UDP / QUIC / TLS |
| **misc** | `tasks`, `bench_loaded`, `share_demo`, `redis_demo`, `storage_demo` | Task queue + Console CLI + `[schedule.*]` / load / Redis / S3·MinIO storage |
