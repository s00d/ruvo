//! Generate **static** VitePress markdown from Ruvo Rust sources.
//!
...
//!
//! ```bash
//! cargo run -p ruvo-docs-gen
//! cargo run -p ruvo-docs-gen -- --check
//! ```
...
#[derive(Parser, Debug)]
#[command(about = "Generate VitePress markdown from Ruvo sources")]
struct Args {
    #[arg(long)]
    check: bool,
    #[arg(long)]
...
fn run(root: &Path, docs: &Path, check: bool) -> Result<bool, String> {
    ...
    let features = parse_features_ordered(
        &fs::read_to_string(root.join("crates/ruvo/Cargo.toml")).map_err(|e| e.to_string())?,
    )?;
    let feature_docs = parse_feature_docs(&root.join("crates/ruvo/src/doc_features.rs"))?;
    ...
    let mut entries: Vec<_> = fs::read_dir(&plugins_dir)
        ...
        .filter(|e| {
            e.file_type().map(|t| t.is_dir()).unwrap_or(false)
                && e.file_name().to_string_lossy().starts_with("ruvo-")
        })
        .collect();
    ...
    for ent in entries {
        ...
        let slug = crate_name.trim_start_matches("ruvo-").to_string();
...
**{summary}** · crate `{crate_name}` · id `{plugin_id}`\n"
    ...
    let nav_ts = format!(
        "// Generated by ruvo-docs-gen — do not edit.\n\
...
export const pluginsSidebar = [\n\
  {{\n\
    ...
    let plugin_sdk = extract_crate_docs(
        &fs::read_to_string(root.join("crates/ruvo-core/src/plugin.rs"))
            .map_err(|e| e.to_string())?,
    )?;
    w.emit(
        "api/plugin-sdk.md",
    ...
    for f in features {
        for d in &f.deps {
            if let Some(crate_name) = d.strip_prefix("dep:") {
                if crate_name.starts_with("ruvo-") {
...
mod tests {
    ...
    fn sanitizes_intra_doc() {
        let s = "UDP as [`BackgroundService`](ruvo_core::BackgroundService).";
