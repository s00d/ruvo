...
pub async fn run(state: Arc<ruvo::extend::StateMap>) -> Result<(), Error> {
    let _pool = state
        .get::<ruvo::DbPool>()
...
pub fn ensure_seeds_registry(snake: &str) -> Result<(), String> {
    ...
    out.push_str("pub async fn run(state: Arc<ruvo::extend::StateMap>) -> Result<(), Error> {\n");
    for m in &mods {
...
pub fn render_crud_dto(fields: Option<&[FieldSpec]>) -> String {
    ...
    let mut out = String::new();
    out.push_str("use ruvo::doc_schema;\n\n");
...
pub fn render_mailer(ty: &str, snake: &str) -> String {
    ...
    format!(
        r#"use ruvo::{{Content, Envelope, Mailable}};
use serde_json::json;
...
<head>
  <meta charset="utf-8">
  <title>{% block title %}Ruvo{% endblock %}</title>
</head>
...
pub fn render_web_handlers(name: &str, plural: &str, fields: Option<&[FieldSpec]>) -> String {
    ...
    out.push_str("}\n\n");
    out.push_str("use ruvo::IntoResponse;\n");
    out
}
...
//!     let app = /* build App */;
//!     let c = ruvo::TestClient::tracked(app).unwrap();
