...
/// {display} plugin.
///
/// Demo install: adds an `x-ruvo-{name}` response header and stores config in app state.
pub struct {ty} {{
    /// Value written to the `x-ruvo-{name}` header.
    header_value: String,
}}
...
impl Plugin for {ty} {{
    ...
    fn meta(&self) -> PluginMeta {{
        PluginMeta::new("{display}")
            .description("Example plugin generated by cargo ruvo generate plugin")
            .version(env!("CARGO_PKG_VERSION"))
            // .sdk(ruvo_core::PLUGIN_SDK_VERSION) // default; pin only if you need an older SDK
    }}
    ...
    fn install(self, app: &mut App) {{
        ...
        app.use_middleware(with_leaked(self, |plugin, req: Request, next| async move {{
            ...
            res = res.header("x-ruvo-{name}", &plugin.header_value);
            res
        }}));
...
{display} plugin for [Ruvo](../../README.md).
...
let mut app = ruvo_core::App::new();
app.install({ty}::new());
```

Declares Plugin SDK via `PluginMeta` (see `ruvo_core::PLUGIN_SDK_VERSION`).
...
"#
    ...
    println!("  use:  app.install(ruvo_{name}::{ty}::new());");
    Ok(())
}
