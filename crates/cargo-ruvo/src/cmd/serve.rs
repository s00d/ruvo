//! `cargo ruvo serve` — run the release binary (production).
...
pub fn run(args: ServeArgs) -> Result<(), String> {
    ...
    if !bin.is_file() {
        if args.no_build {
            return Err(format!(
                "release binary not found at {} (run `cargo ruvo build` or omit --no-build)",
                bin.display()
            ));
        }
        eprintln!("ruvo: release binary missing — building…");
        build::run(BuildArgs {
            package: args.package.clone(),
            manifest_path: args.manifest_path.clone(),
    ...
    eprintln!("ruvo: serve {}", bin.display());
    let mut cmd = Command::new(&bin);
    cmd.current_dir(&project.package_dir)
        .env("RUVO_ENV", "production")
        .stdin(Stdio::inherit())
