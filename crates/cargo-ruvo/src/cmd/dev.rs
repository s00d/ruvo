...
pub fn run(args: DevArgs) -> Result<(), String> {
    ...
    eprintln!(
        "ruvo: dev -p {} ({mode}; restart on .rs / Cargo.toml / .env / ruvo.toml)",
...
fn restart_graceful(
    ...
    let new = match spawn_rust_with_id(project, true, &instance_id) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("ruvo: restart failed (keeping old process): {e}");
            return;
        }
    };
    ...
    eprintln!(
        "ruvo: waiting for new process (instance={instance_id}) on :{port}/ready …"
    );
    if !wait_ready(port, &instance_id, READY_DEADLINE, stop) {
        ...
        eprintln!("ruvo: new process not ready in time — aborting restart, killing new");
        let mut new = new;
        kill_graceful(&mut new, drain);
        return;
    }
    ...
    eprintln!(
        "ruvo: new process ready — draining old ({}s)…",
        drain.as_secs()
    );
...
fn spawn_rust_with_id(
    ...
    cmd.args(["run", "-p", &project.package_name, "--manifest-path"])
        ...
        .env("RUVO_INSTANCE_ID", instance_id)
        .stdin(Stdio::null())
        .stdout(Stdio::inherit())
        .stderr(Stdio::inherit());

    if reuseport {
        cmd.env("RUVO_REUSEPORT", "1");
    }
...
/// Poll until `GET /ready` returns 2xx with matching `x-ruvo-instance`.
///
