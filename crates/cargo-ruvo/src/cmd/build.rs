//! `cargo ruvo build` — optional frontend build + `cargo build --release`.
...
pub fn run(args: BuildArgs) -> Result<(), String> {
    ...
    if !args.no_frontend {
        if let Some(ref fe) = project.frontend {
            ...
            eprintln!("ruvo: no frontend detected — skipping asset build");
        }
    }

    eprintln!(
        "ruvo: cargo build --release -p {}",
    ...
    if status.success() {
        eprintln!("ruvo: release binary → {}", project.release_bin().display());
        Ok(())
    } else {
