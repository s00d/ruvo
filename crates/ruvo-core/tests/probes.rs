use ruvo_core::{App, CheckKind, Error, TestClient};
...
async fn ready_exposes_instance_header() {
    std::env::set_var("RUVO_INSTANCE_ID", "test-instance-1");
    let mut app = App::new();
    app.with_probes();
    let c = TestClient::tracked(app).unwrap();
    let res = c.get("/ready").await;
    assert_eq!(res.status_code().as_u16(), 200);
    assert_eq!(
        res.headers().get("x-ruvo-instance").and_then(|v| v.to_str().ok()),
        Some("test-instance-1")
    );
    std::env::remove_var("RUVO_INSTANCE_ID");
}
