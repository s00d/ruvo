//! Resolve the Cargo package and optional `[frontend]` from `sova.toml`.
...
#[derive(Debug, Default, Deserialize)]
struct SovaToml {
    frontend: Option<SovaTomlFrontend>,
}
...
fn load_sova_toml(dirs: &[&Path]) -> Option<SovaToml> {
    for dir in dirs {
        for name in ["sova.toml", "Sova.toml"] {
            let path = dir.join(name);
            if path.is_file() {
                if let Ok(text) = fs::read_to_string(&path) {
                    if let Ok(doc) = toml::from_str::<SovaToml>(&text) {
                        return Some(doc);
                    }
                }
            }
...
fn resolve_frontend(
    ...
    let sova = load_sova_toml(&[package_dir, workspace_dir]);
    let section = sova.and_then(|r| r.frontend);
