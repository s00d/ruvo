//! Load app-level settings from `sova.toml` (no per-route addressing).
//!
//! # Document shape
//!
//! ```toml
...
//! ```
//!
//! Active profile: `SOVA_PROFILE` → else `SOVA_ENV` → else
...
fn active_profile() -> String {
    if let Ok(p) = std::env::var("SOVA_PROFILE") {
        return normalize_profile_name(&p);
    }
    if let Ok(p) = std::env::var("SOVA_ENV") {
        return normalize_profile_name(&p);
    }
...
fn env_override(app: &mut App) -> Result<()> {
    if let Ok(s) = std::env::var("SOVA_MAX_BODY") {
        app.max_body_size(parse_bytes(&s).map_err(Error::Internal)?);
    }
    if let Ok(s) = std::env::var("SOVA_MAX_CONNECTIONS") {
        let n: usize = s
            .parse()
            .map_err(|_| Error::Internal(format!("SOVA_MAX_CONNECTIONS: {s}")))?;
        app.max_connections(n);
    }
    ...
    if let Ok(s) = std::env::var("SOVA_MAX_HEADERS") {
        let n: usize = s
            .parse()
            .map_err(|_| Error::Internal(format!("SOVA_MAX_HEADERS: {s}")))?;
        app.max_headers(n);
    }
    if let Ok(s) = std::env::var("SOVA_MAX_BUF_SIZE") {
        app.max_buf_size(parse_bytes(&s).map_err(Error::Internal)?);
    }
    if let Ok(s) = std::env::var("SOVA_REQUEST_TIMEOUT") {
        app.request_timeout(parse_timeout_env(&s)?);
    }
    if let Ok(s) = std::env::var("SOVA_HEADER_READ_TIMEOUT") {
        app.header_read_timeout(parse_duration(&s).map_err(Error::Internal)?);
    }
    if let Ok(s) = std::env::var("SOVA_IDLE_TIMEOUT") {
        app.idle_timeout(parse_duration(&s).map_err(Error::Internal)?);
    }
    if let Ok(s) = std::env::var("SOVA_DRAIN_TIMEOUT") {
        app.drain_timeout(parse_duration(&s).map_err(Error::Internal)?);
    }
    if let Ok(s) = std::env::var("SOVA_KEEP_ALIVE") {
        let v = matches!(s.as_str(), "1" | "true" | "TRUE" | "yes");
        app.keep_alive(v);
    }
    if let Ok(s) = std::env::var("SOVA_TRUST_PROXY") {
        let v = matches!(s.as_str(), "1" | "true" | "TRUE" | "yes");
        app.trust_proxy(v);
    }
    Ok(())
...
impl App {
    ...
    pub fn configure(&mut self) -> Result<&mut Self> {
        for name in ["sova.toml", "Sova.toml"] {
