...
fn try_from_vars_file_and_explicit_fake() {
    ...
    let mail = Mail::try_from_vars(|k| match k {
        "SOVA_MAIL" => Some("file".into()),
        "SOVA_MAIL_PATH" => Some(path.clone()),
        "SOVA_MAIL_FROM" => Some("File <f@t.local>".into()),
        _ => None,
    })
    .unwrap();
    ...
    let fake = Mail::try_from_vars(|k| match k {
        "SOVA_MAIL" => Some("fake".into()),
        "SOVA_MAIL_FROM" => Some("Fake <f@t.local>".into()),
        _ => None,
    })
    .unwrap();
...
fn try_from_vars_smtp_requires_url() {
    ...
    assert!(err.to_string().contains("SOVA_MAIL_URL") || err.to_string().contains("smtp"));
}
...
async fn file_from_vars_sends_eml() {
    ...
    let mail = Mail::try_from_vars(|k| match k {
        "SOVA_MAIL" => Some("file".into()),
        "SOVA_MAIL_PATH" => Some(path.clone()),
        "SOVA_MAIL_FROM" => Some("App <a@t.local>".into()),
        _ => None,
    })
    .unwrap();
    let client = mail.client();
