...
use sova_core::{with_state, App, Error, Plugin, RateLimitIdentity, Request, Result};
...
impl Plugin for Passport {
    ...
    fn meta(&self) -> sova_core::PluginMeta {
        sova_core::PluginMeta::new("Passport")
    ...
    fn install(self, app: &mut App) {
        ...
        if needs_session {
            app.on_startup(|_s| async {
                Err(Error::Internal(
                    "Passport serialize/deserialize requires feature `session` on sova-passport"
                        .into(),
                ))
            });
            return;
