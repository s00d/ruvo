...
use http::Method;
use ruvo_core::extend::{BoxFuture, RouteEntry, RouteTable, StateMap};
use std::sync::Arc;
...
pub fn should_include_route(method: &Method, path: &str, meta: &ruvo_core::extend::MetaMap) -> bool {
    ...
    #[cfg(feature = "openapi")]
    {
        if let Some(doc) = meta.get::<ruvo_openapi::Doc>() {
            if !doc.is_skip() {
                return false;
            }
        }
...
fn append_xhtml_links(out: &mut String, public_url: &str, entry_path: &str, h: &HreflangOpts) {
    #[cfg(feature = "i18n")]
    {
        use ruvo_i18n::{localized_url, strip_locale_prefix};
