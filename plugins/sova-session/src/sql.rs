...
impl SqlSessionStore {
    ...
    async fn ensure_schema_on(&self, conn: &DatabaseConnection) -> Result<(), DbErr> {
        ...
        let ddl = match backend {
            DbBackend::Postgres => {
                r#"
                CREATE TABLE IF NOT EXISTS sova_sessions (
                    id text PRIMARY KEY,
                    user_id text,
                    payload bytea NOT NULL,
                    expires_at text
                );
                CREATE INDEX IF NOT EXISTS sova_sessions_user_id_idx
                    ON sova_sessions (user_id);
                CREATE INDEX IF NOT EXISTS sova_sessions_expires_at_idx
                    ON sova_sessions (expires_at);
                "#
            }
            DbBackend::MySql => {
                r#"
                CREATE TABLE IF NOT EXISTS sova_sessions (
                    ...
                    INDEX sova_sessions_user_id_idx (user_id),
                    INDEX sova_sessions_expires_at_idx (expires_at)
                )
                "#
            }
            _ => {
...
fn upsert_sql(backend: DbBackend) -> &'static str {
    match backend {
        DbBackend::MySql => {
            r#"
            INSERT INTO sova_sessions (id, user_id, payload, expires_at)
            VALUES ($1, $2, $3, $4)
    ...
    fn load<'a>(&'a self, id: &'a str) -> BoxFuture<'a, Option<HashMap<String, String>>> {
        Box::pin(async move {
            ...
            let stmt = Statement::from_sql_and_values(
                backend,
                "SELECT payload, expires_at FROM sova_sessions WHERE id = $1",
                [val_key(id)],
            );
            ...
            if expired(&exp) {
                let _ = conn
                    .execute_raw(Statement::from_sql_and_values(
                        backend,
                        "DELETE FROM sova_sessions WHERE id = $1",
                        [val_key(id)],
                    ))
                    .await;
                return None;
            ...
            let result = match keep_sid {
                Some(keep) => {
                    conn.execute_raw(Statement::from_sql_and_values(
                        backend,
                        "DELETE FROM sova_sessions WHERE user_id = $1 AND id <> $2",
                ...
                None => {
                    conn.execute_raw(Statement::from_sql_and_values(
                        backend,
                        "DELETE FROM sova_sessions WHERE user_id = $1",
                        [val_key(user_id)],
                    ))
                    .await
                }
