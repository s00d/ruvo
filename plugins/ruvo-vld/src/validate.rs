...
use ruvo_core::extend::{named, BoxFuture, MwEntry, RouteTable, RouteValue};
use ruvo_core::{App, Request, Response, Router};
    ...
    pub fn form<T: VldParse + Send + Sync + 'static>() -> Self {
        ...
        #[cfg(not(feature = "form"))]
        {
            ...
            Self::wrap(|_req| {
                Box::pin(async move {
                    Err(Response::text(
                        "Validate::form requires ruvo-vld feature `form`",
                    )
                    .status(500))
                })
            })
...
fn vld_mw() -> MwEntry {
    named("vld", |req: Request, next: ruvo_core::Next| async move {
...
fn attach_openapi(router: &mut Router, source: ValidateSource, schema: Value) {
    use ruvo_openapi::OpenApiValidate;
...
pub fn missing_validate_routes(table: &RouteTable) -> Vec<String> {
    ...
    for entry in &table.0 {
        let ruvo_core::extend::RouteEntry::Http {
            method,
            path,
            meta,
        } = entry
    ...
    fn install(self, app: &mut App) {
        app.register_audit("vld", |state| async move {
            ...
            if missing.is_empty() {
                ...
                Err(ruvo_core::Error::Internal(format!(
