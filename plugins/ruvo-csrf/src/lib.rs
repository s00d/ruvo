...
use http::Method;
use ruvo_cookies::{CookieBuilder, ResponseCookieExt};
use ruvo_core::extend::named;
use ruvo_core::{with_state, App, Error, Plugin, Request, Response};
...
pub trait CsrfExt {
    ...
    fn verify_csrf(&self, submitted: Option<&str>) -> ruvo_core::Result<()>;
}
...
async fn submitted_token(req: &mut Request, cfg: &Csrf) -> ruvo_core::Result<TokenFind> {
