...
use sova_core::{BackgroundService, Error, Result, Shutdown};
...
pub type QuicDatagramHandler =
    Arc<dyn Fn(SocketAddr, Vec<u8>, quinn::Connection) -> SovaBoxFuture<()> + Send + Sync>;
...
mod tests {
    ...
    async fn echo_roundtrip() {
        ...
        let server_alpn = vec![b"sova_quic-udp".to_vec()];
