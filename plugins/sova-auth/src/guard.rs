...
use sova_core::{with_state, Error, IntoResponse, Next, Redirect, Request, Response, Result};
use sova_passport::PassportExt;
...
impl AuthExt for Request {
    ...
    async fn logout_all_sessions(&mut self) -> Result<u64> {
        ...
        let _ = self.take::<CurrentUser>();
        let _ = self.take::<sova_passport::Authenticated>();
        Ok(n)
    }
}
