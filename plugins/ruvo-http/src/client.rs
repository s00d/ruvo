...
/// Per-named-client overrides from `ruvo.toml`.
...
impl Http {
    ...
    /// Merge named clients from [`ruvo_core::ConfigDoc`] already on `app`.
...
fn parse_named(table: &toml::map::Map<String, toml::Value>) -> NamedClientConfig {
    use ruvo_core::extend::parse_bytes;
    use ruvo_core::extend::parse_duration;
