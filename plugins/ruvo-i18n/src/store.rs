...
pub fn load_store(dir: &Path, locales: &[Locale]) -> ruvo_core::Result<Store> {
    ...
    for loc in locales {
        ...
        if pages_dir.is_dir() {
            for entry in std::fs::read_dir(&pages_dir).map_err(ruvo_core::Error::from)? {
                let entry = entry.map_err(ruvo_core::Error::from)?;
...
fn read_json_object(path: &Path) -> ruvo_core::Result<Value> {
    if !path.is_file() {
        return Ok(Value::Object(Map::new()));
    }
    let raw = std::fs::read_to_string(path).map_err(ruvo_core::Error::from)?;
    let v: Value = serde_json::from_str(&raw)
        .map_err(|e| ruvo_core::Error::Internal(format!("i18n {}: {e}", path.display())))?;
    if !v.is_object() {
...
fn build_scope(tree: &Value) -> ruvo_core::Result<Scope> {
    let mut flat = HashMap::new();
    flatten(tree, "", &mut flat);
    let payload = Bytes::from(serde_json::to_vec(tree).map_err(|e| {
        ruvo_core::Error::Internal(format!("i18n serialize: {e}"))
    })?);
