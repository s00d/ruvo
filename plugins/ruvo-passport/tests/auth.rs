...
use http::Method;
use ruvo_core::{App, Json, Plugin, Request, Response};
use ruvo_passport::{Auth, Extract, Passport, PassportExt};
...
async fn api_key_via_passport() {
    ...
    app.get("/me", |req: Request| async move {
        Ok::<_, ruvo_core::Error>(Json(req.require_user::<User>()?.clone()))
    });
...
fn password_and_refresh_hash() {
    use ruvo_passport::{hash_password, hash_refresh_token, verify_password};
...
fn oauth_state_and_pkce_roundtrip() {
    use ruvo_passport::oauth_test_support::{
...
fn oauth_profile_parse() {
    use ruvo_passport::oauth_test_support::parse_profile;
    use ruvo_passport::ProfileKind;
    use serde_json::json;
...
async fn passport_login_logout_session() {
    use ruvo_passport::{Authenticated, Passport};
