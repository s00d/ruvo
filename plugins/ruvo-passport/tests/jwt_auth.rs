...
use ruvo_core::{Json, Request, ResponseAssert, Router, TestClient};
use ruvo_passport::{AuthMigrator, JwtAuth, JwtAuthExt};
    ...
    let (_tdb, app) = TestApp::builder()
        ...
        .configure(|app| {
            ...
            api.get("/me", |req: Request| async move {
                Ok::<_, ruvo_core::Error>(Json(req.require_auth_user()?.clone()))
            });
            app.mount("/api", api);
        })
        .build()
    ...
    let (_tdb, app) = TestApp::builder()
        ...
        .configure(|app| {
            ...
            api.get("/who", |req: Request| async move {
                ...
                Ok::<_, ruvo_core::Error>(Json(serde_json::json!({ "via": via })))
            });
            app.mount("/api", api);
        })
        .build()
