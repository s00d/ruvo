//! SQL-backed [`TaskStore`] on the shared [`ruvo_db::DbPool`] (postgres / sqlite / mysql).
            ...
            if r.rows_affected() == 0 {
                let exists = conn
                    .query_one_raw(Statement::from_sql_and_values(
                        backend,
                        "SELECT status FROM ruvo_tasks WHERE id = $1",
                        [val_str(id)],
                    ))
                    .await
                    .map_err(map_err)?;
        ...
        Box::pin(async move {
            ...
            for row in rows {
                ...
                let full = txn
                    .query_one_raw(Statement::from_sql_and_values(
                        backend,
                        format!("SELECT {} FROM ruvo_tasks WHERE id = $1", select_cols()),
                        [val_str(id)],
                    ))
                    .await?
