//! Redis-backed [`TaskStore`] on the shared [`ruvo_redis::RedisPool`].
...
const SEQ_KEY: &str = "ruvo:t:seq";
const LEASE_ZSET: &str = "ruvo:t:lease";

fn ready_key(queue: &str) -> String {
    format!("ruvo:t:{queue}:ready")
}

fn all_key(queue: &str) -> String {
    format!("ruvo:t:{queue}:all")
}
...
for _, id in ipairs(ids) do
  ...
  if queue then
    ...
    redis.call('ZADD', 'ruvo:t:' .. queue .. ':ready', score, id)
    n = n + 1
  else
    redis.call('ZREM', lease, id)
  end
