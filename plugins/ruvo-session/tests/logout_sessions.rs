...
use ruvo_session::{memory_sessions, SessionExt, SessionLayer, SessionStoreHandle};
use ruvo_store::{namespace, MemoryStore};
use std::sync::Arc;

fn app_with_store(store: Arc<dyn ruvo_store::KvStore>) -> App {
    let mut app = App::new();
...
mod sql_logout {
    use super::*;
    use ruvo_db::DbPool;
    use ruvo_session::SqlSessionStore;
...
mod redis_logout {
    ...
    use ruvo_session::RedisSessionStore;
