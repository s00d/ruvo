...
use bytes::Bytes;
use rand::RngCore;
use ruvo_core::{App, Plugin, Request, Upload};
...
impl AppStorage {
    ...
    ///
    /// Validate with [`ruvo_core::UploadRules`] before calling.
...
impl Storage {
    ...
    pub fn from_config(app: &App) -> Result<Self, StorageError> {
        ...
        let mut storage = match driver.as_str() {
            "local" => {
                #[cfg(feature = "local")]
                {
                    let path = std::env::var("RUVO_STORAGE_PATH")
                        .ok()
                        .filter(|s| !s.is_empty())
                        .or_else(|| {
        ...
        let public = std::env::var("RUVO_STORAGE_PUBLIC_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                section.as_ref().and_then(|s| {
    ...
    /// Build from `RUVO_STORAGE` (`local` | `memory` | `s3` | `gcs` | `azure`).
    pub fn from_env() -> Result<Self, StorageError> {
        let kind = std::env::var("RUVO_STORAGE").unwrap_or_else(|_| "local".into());
        let mut storage = match kind.as_str() {
            "local" => {
                ...
                {
                    Err(StorageError::Msg(
                        "RUVO_STORAGE=local requires feature `local`".into(),
                    ))
                }
            }
            "memory" => {
                ...
                {
                    Err(StorageError::Msg(
                        "RUVO_STORAGE=memory requires feature `memory`".into(),
                    ))
                }
            }
            "s3" => {
                ...
                {
                    Err(StorageError::Msg(
                        "RUVO_STORAGE=s3 requires feature `s3` (facade: storage-s3)".into(),
                    ))
                }
            }
            "gcs" => {
                ...
                {
                    Err(StorageError::Msg(
                        "RUVO_STORAGE=gcs requires feature `gcs` (facade: storage-gcs)".into(),
                    ))
                }
            }
            "azure" => {
                ...
                {
                    Err(StorageError::Msg(
                        "RUVO_STORAGE=azure requires feature `azure` (facade: storage-azure)"
                            .into(),
                    ))
                }
            }
            other => Err(StorageError::Msg(format!(
                "unknown RUVO_STORAGE={other} (use local|memory|s3|gcs|azure)"
            ))),
        }?;

        if let Ok(base) = std::env::var("RUVO_STORAGE_PUBLIC_URL") {
            storage = storage.public_url(base);
        }
        Ok(storage)
    }
...
impl TomlEnvGuard {
    fn apply(section: Option<&toml::map::Map<String, toml::Value>>) -> Self {
        ...
        let pairs = [
            ("bucket", "RUVO_STORAGE_BUCKET"),
            ("region", "RUVO_STORAGE_REGION"),
            ("endpoint", "RUVO_STORAGE_ENDPOINT"),
            ("root", "RUVO_STORAGE_ROOT"),
            ("force_path_style", "RUVO_STORAGE_FORCE_PATH_STYLE"),
            ("path", "RUVO_STORAGE_PATH"),
            ("public_url", "RUVO_STORAGE_PUBLIC_URL"),
        ];
...
impl Plugin for Storage {
    ...
    fn meta(&self) -> ruvo_core::PluginMeta {
        ruvo_core::PluginMeta::new("Storage")
    ...
    fn install(mut self, app: &mut App) {
        ...
        app.register_check("storage", move |_state| {
            ...
            async move {
                ...
                check
                    .exists("__ruvo_storage_health__")
                    .await
                    .map(|_| ())
                    .map_err(|e| ruvo_core::Error::Internal(e.to_string()))
            }
        });
    }
}
