...
use bytes::Bytes;
use ruvo_core::{App, Request, Upload};
use ruvo_storage::{AppStorage, PutOpts, Storage, StorageExt};
...
fn clear_storage_env() {
    for k in [
        ...
        "RUVO_STORAGE_BUCKET",
        "RUVO_STORAGE_REGION",
        "RUVO_STORAGE_ENDPOINT",
        "RUVO_STORAGE_ROOT",
        "RUVO_STORAGE_FORCE_PATH_STYLE",
    ] {
...
async fn from_env_memory_and_local() {
    ...
    std::env::set_var("RUVO_STORAGE_PUBLIC_URL", "https://x.test");
    ...
    std::env::set_var("RUVO_STORAGE", "local");
    std::env::set_var("RUVO_STORAGE_PATH", dir.path().to_str().unwrap());
    std::env::remove_var("RUVO_STORAGE_PUBLIC_URL");
    ...
    std::env::set_var("RUVO_STORAGE", "nope");
