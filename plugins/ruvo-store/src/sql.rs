//! SQL-backed [`KvStore`] on the shared [`ruvo_db::DbPool`] (postgres / sqlite / mysql).
...
/// Key-value store on table `ruvo_kv`, using the app [`DbPool`].
#[derive(Clone)]
pub struct SqlStore {
    pool: DbPool,
...
impl SqlStore {
    ...
    /// Bind to Ruvo [`DbPool`]. Schema is created on first use (pool may connect later).
...
fn upsert_sql(backend: DbBackend) -> String {
    ...
    match backend {
        DbBackend::MySql => format!(
            r#"
            INSERT INTO ruvo_kv ({k}, value, expires_at)
            VALUES ($1, $2, $3)
...
fn select_sql(backend: DbBackend) -> String {
    let k = key_col(backend);
    format!("SELECT value, expires_at FROM ruvo_kv WHERE {k} = $1")
}

fn delete_sql(backend: DbBackend) -> String {
    let k = key_col(backend);
    format!("DELETE FROM ruvo_kv WHERE {k} = $1")
}

fn delete_prefix_sql(backend: DbBackend) -> String {
    let k = key_col(backend);
    format!("DELETE FROM ruvo_kv WHERE {k} LIKE $1")
}
