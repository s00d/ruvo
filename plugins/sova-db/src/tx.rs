...
pub fn transaction() -> impl sova_core::extend::IntoMwEntry {
...
impl IntoResponseViaError for DbError {
    fn into_response_via_error(self) -> Response {
        sova_core::IntoResponse::into_response(self)
    }
}

/// Helper used by plugin install to inject Conn handle.
pub(crate) fn inject_conn(pool: DbPool) -> impl sova_core::extend::IntoMwEntry {
    named(
        "db",
        sova_core::with_state(pool, |pool, mut req, next| async move {
            match pool.get() {
                ...
                Err(err) => sova_core::IntoResponse::into_response(err),
            }
        }),
    )
}
