...
#[cfg(feature = "s3")]
impl S3EnvConfig {
    /// Read process env (`SOVA_STORAGE_*` / `AWS_*`).
    ...
    {
        let bucket = get("SOVA_STORAGE_BUCKET")
        ...
        let region = get("SOVA_STORAGE_REGION")
            .or_else(|| get("AWS_REGION"))
            .filter(|s| !s.is_empty())
            .or_else(|| endpoint.as_ref().map(|_| "auto".to_string()))
            .ok_or_else(|| {
                StorageError::Msg(
                    "SOVA_STORAGE_REGION or AWS_REGION is required for s3 \
                     (or set SOVA_STORAGE_ENDPOINT for MinIO/R2 to default region=auto)"
                        .into(),
                )
            })?;

        // Default: path-style when a custom endpoint is set (MinIO); virtual-host for plain AWS.
        let force_path_style = match get("SOVA_STORAGE_FORCE_PATH_STYLE") {
            Some(raw) if env_truthy(&raw) => true,
            Some(raw) if env_falsy(&raw) => false,
            Some(raw) => {
                return Err(StorageError::Msg(format!(
                    "invalid SOVA_STORAGE_FORCE_PATH_STYLE={raw} (use 1/0, true/false)"
                )));
            }
        ...
        Ok(Self {
            ...
            session_token: get("AWS_SESSION_TOKEN")
                .or_else(|| get("SOVA_STORAGE_SESSION_TOKEN"))
                .filter(|s| !s.is_empty()),
            root: get("SOVA_STORAGE_ROOT").filter(|s| !s.is_empty()),
            force_path_style,
        })
    }
...
pub fn gcs_from_env() -> Result<OpendalStore, StorageError> {
    ...
    if let Some(root) = env_get("SOVA_STORAGE_ROOT") {
        builder = builder.root(&root);
    }
    if let Some(endpoint) = env_get("SOVA_STORAGE_ENDPOINT") {
        builder = builder.endpoint(&endpoint);
    }
...
pub fn azure_from_env() -> Result<OpendalStore, StorageError> {
    ...
    let container = env_get("SOVA_STORAGE_CONTAINER")
        .or_else(|| env_get("SOVA_STORAGE_BUCKET"))
        .ok_or_else(|| {
            StorageError::Msg(
                "SOVA_STORAGE_CONTAINER (or SOVA_STORAGE_BUCKET) required for azure".into(),
            )
        })?;
    ...
    // Azurite / custom: AZURE_STORAGE_ENDPOINT preferred, else SOVA_STORAGE_ENDPOINT.
    if let Some(endpoint) =
        env_get("AZURE_STORAGE_ENDPOINT").or_else(|| env_get("SOVA_STORAGE_ENDPOINT"))
    {
        builder = builder.endpoint(&endpoint);
    }
    if let Some(root) = env_get("SOVA_STORAGE_ROOT") {
...
mod s3_env_tests {
    ...
    fn minio_endpoint_defaults_region_auto_and_path_style() {
        ...
        env.insert("SOVA_STORAGE_BUCKET", "demo");
        env.insert("SOVA_STORAGE_ENDPOINT", "http://127.0.0.1:9000");
    ...
    fn force_path_style_override_and_session_token() {
        ...
        env.insert("SOVA_STORAGE_FORCE_PATH_STYLE", "1");
        env.insert("AWS_SESSION_TOKEN", "tok");
        env.insert("SOVA_STORAGE_ROOT", "uploads");
    ...
    fn empty_bucket_rejected() {
        let mut env = HashMap::new();
        env.insert("SOVA_STORAGE_BUCKET", "");
        env.insert("AWS_REGION", "us-east-1");
        let err = S3EnvConfig::from_vars(map_get(&env)).unwrap_err();
