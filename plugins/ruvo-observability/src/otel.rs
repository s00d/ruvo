...
///
/// Env: `OTEL_EXPORTER_OTLP_ENDPOINT` (required to enable), `OTEL_SERVICE_NAME` (default `ruvo`).
pub fn install_from_env() -> Result<(), String> {
    ...
    let service = std::env::var("OTEL_SERVICE_NAME").unwrap_or_else(|_| "ruvo".into());
    ...
    let tracer = provider.tracer("ruvo");
