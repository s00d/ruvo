...
impl ElasticsearchLog {
    /// From env: `ELASTICSEARCH_URL` (required), `ELASTICSEARCH_INDEX` (default `sova-logs`),
    /// `ELASTICSEARCH_API_KEY` or `ELASTICSEARCH_USERNAME`/`ELASTICSEARCH_PASSWORD`,
    /// `SOVA_SERVICE_NAME` / `OTEL_SERVICE_NAME` for `service` field.
    pub fn from_env() -> Result<Self, String> {
        ...
        let service = std::env::var("SOVA_SERVICE_NAME")
            .or_else(|_| std::env::var("OTEL_SERVICE_NAME"))
            .unwrap_or_else(|_| "sova".into());
