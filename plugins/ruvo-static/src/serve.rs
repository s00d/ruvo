...
pub(crate) async fn serve_in(dir: &Path, relative: &Path, opts: FileOptions<'_>) -> Response {
    ...
    if not_modified(&etag, modified, opts.if_none_match, opts.if_modified_since) {
        ...
        set_file_headers(&mut res, &mime, &etag, modified, None);
        return res;
    }

    if let Some(range) = opts.range.and_then(parse_byte_range) {
        if let Some((start, end)) = range.resolve(len) {
            return serve_range(&full, &mime, &etag, modified, start, end, len).await;
        }
    ...
    // Full body via core stream primitive, then attach validators / cache headers.
    let mut res = Response::file_in(dir, relative).await;
    if res.status_code().as_u16() != 200 {
        return res;
    }
    set_file_headers(&mut res, &mime, &etag, modified, Some(len));
    res
}
...
async fn serve_range(
    ...
    set_file_headers(&mut res, mime, etag, modified, Some(end - start + 1));
