...
use http::Method;
use sova_core::{App, CheckKind, Request, Response};
use sova_meta::{Meta, Robots};
use std::sync::Mutex;

/// Serialize env mutation — `SOVA_PROFILE` is process-global.
...
async fn audit_meta_profile(app: App, profile: Option<&str>) -> (bool, Option<String>) {
    let (_server, state) = {
        ...
        match profile {
            Some(p) => std::env::set_var("SOVA_PROFILE", p),
