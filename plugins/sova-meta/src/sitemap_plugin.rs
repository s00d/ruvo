...
impl Plugin for Sitemap {
    ...
    fn meta(&self) -> sova_core::PluginMeta {
        sova_core::PluginMeta::new("Sitemap")
    ...
    fn install(mut self, app: &mut App) {
        ...
        let kv: Option<Arc<dyn KvStore>> = self.cache_store.take().or_else(|| {
            app.try_state::<sova_store::AppStore>()
...
fn hreflang_from_req(req: &Request) -> Option<HreflangOpts> {
    #[cfg(feature = "i18n")]
    {
        let state = req.try_state::<sova_i18n::I18nState>()?;
...
fn state_from_req(req: &Request) -> Arc<sova_core::extend::StateMap> {
    let mut map = sova_core::extend::StateMap::new();
    ...
    #[cfg(feature = "i18n")]
    if let Some(i18n) = req.try_state::<sova_i18n::I18nState>() {
