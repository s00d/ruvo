...
use ruvo_core::{
    BackgroundService, Error, Request as RuvoRequest, Result, Server as RuvoServer, Shutdown,
};
...
/// `ruvo_core::Server` and writes responses back over h3.
...
async fn handle_h3_connection(conn: quinn::Connection, app_server: RuvoServer, shutdown: Shutdown) {
                        ...
                        tokio::spawn(async move {
                            match resolver.resolve_request().await {
                                Ok((req, mut stream)) => {
                                    ...
                                    let mut ruvo_req = RuvoRequest::builder()
                                    ...
                                    for (name, value) in req.headers() {
                                        ruvo_req.headers.insert(name.clone(), value.clone());
                                    }

                                    let mut res = app_server.handle(ruvo_req).await;
...
mod tests {
    ...
    #[tokio::test]
    async fn h3_service_dispatches_ruvo_routes() {
