//! Ruvo stand: same bodies as axum/actix (see `stand_fixtures`).
//! Minimal stack — no logger middleware (fair vs other frameworks).

use ruvo::{App, Html, IntoResponse, Request, Response, Result};
use stand_fixtures::{
...
async fn main() -> Result<()> {
    ...
    eprintln!("stand_ruvo listening on http://127.0.0.1:{port}");
